...
import { Bell, BellOff, Check, Trash2, Calendar, ShieldCheck, AlertTriangle, Clock } from 'lucide-react';
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5 select-none">
        <p className="text-xs font-bold text-slate-400">
          Check up-to-date schedule changes, credential approvals, and console logs.
        </p>
        {notifications.some(n => !n.isRead) && (
          <Button 
            onClick={handleMarkAllReadClick} 
            variant="outline" 
            className="py-2 px-4 text-xs rounded-xl font-bold shadow-xs hover:-translate-y-0.5 active:translate-y-0 bg-white"
          >
            Mark all read
          </Button>
        )}
